...
for subdir in "$pathToClean"/*/
do
  # Get the name of the current subdirectory
  subdir_name="${subdir##*/}"

  # Replace any illegal characters in the subdirectory name with an underscore
  new_subdir_name=$(echo "$subdir_name" | tr -cd '[:alnum:]_.')

  # Only rename the subdirectory if the new name is different from the old name
  if [ "$subdir_name" != "$new_subdir_name" ]
  then
    # Rename the subdirectory
    mv "$subdir" "$dir/$new_subdir_name"
  fi
done
}
